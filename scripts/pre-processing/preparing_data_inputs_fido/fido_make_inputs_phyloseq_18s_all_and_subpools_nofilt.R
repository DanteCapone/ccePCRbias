...
taxa_18s=read.csv(here("PCR_bias_correction/data/taxa_files/blast_metazoo_18s_qc.csv")) %>% 
  # select(-X) %>% 
...
experiments <- c("AllPool", "Pooled.A1.A3", "Pooled.C5.C7")
...
fido_18s_s1_final %>%
  ...
  group_by(Family) %>% 
  summarise(across(where(is.numeric), sum, na.rm = TRUE))->fido_18s_s1_save_family_phy_nofilt

fido_18s_s1_save_family_phy_nofilt%>% 
...
write.csv(fido_18s_s1_save_family_phy_nofilt, 
          here("PCR_bias_correction/data/fido/phy/fido_18s_s1_family_phy_all_subpools_nofilt.csv"), 
          row.names = FALSE)
...
fido_18s_s2_final %>%
  ...
  select(-Phylum, -Class, -Genus, -Order, -Species, -Hash) %>%
  group_by(Family) %>% 
  summarise(across(where(is.numeric), sum, na.rm = TRUE))->fido_18s_s2_save_family_phy_nofilt

fido_18s_s2_save_family_phy_nofilt%>% 
  pivot_longer(cols = -Family, names_to = "Category", values_to = "Value") %>% 
  group_by(Family, Category) %>% 
  summarize(taxa_sum = sum(Value), .groups = 'drop') %>% 
  ungroup() %>%  group_by(Category) %>% 
...
# Save cleaned dataset
write.csv(fido_18s_s2_save_family_phy_nofilt, 
          here("PCR_bias_correction/data/fido/phy/fido_18s_s2_family_phy_all_subpools_nofilt.csv"), 
          row.names = FALSE)
...
fido_18s_s3_final %>%
  ...
  select(-Phylum, -Class, -Genus, -Order, -Species, -Hash) %>%
  group_by(Family) %>% 
  summarise(across(where(is.numeric), sum, na.rm = TRUE))->fido_18s_s3_save_family_phy_nofilt

#Save
write.csv(fido_18s_s3_save_family_phy_nofilt,here("PCR_bias_correction/data/fido/phy/fido_18s_s3_family_phy_all_subpools_nofilt.csv"))

fido_18s_s3_save_family_phy_nofilt%>% 
  pivot_longer(cols = -Family, names_to = "Category", values_to = "Value") %>% 
  group_by(Family, Category) %>% 
  summarize(taxa_sum = sum(Value), .groups = 'drop') %>% 
  ungroup() %>%  group_by(Category) %>% 
...
final_plot1
# Save first plot
save_path1 <- here("figures/miscellaneous/calibration_experiment_taxa_proportions_18s_all_samples")
...
# Save second plot
save_path2 <- here("figures/miscellaneous/calibration_experiment_taxa_proportions_18s")
